...
from transformers_local.utils.logging import tqdm
from transformers_local.utils.versions import importlib_metadata
...
old_default_cache_path = os.path.join(torch_cache_home, "transformers_local")
# New default cache, shared with the Datasets library
hf_cache_home = os.path.expanduser(
    os.getenv("HF_HOME", os.path.join(os.getenv("XDG_CACHE_HOME", "~/.cache"), "huggingface"))
)
default_cache_path = os.path.join(hf_cache_home, "transformers_local")
...
if (
    ...
    logger.warning(
        ...
        "'~/.cache/torch/transformers_local' to '~/.cache/huggingface/transformers_local'. Since you don't seem to have overridden "
        "and '~/.cache/torch/transformers_local' is a directory that exists, we're moving it to "
        "'~/.cache/huggingface/transformers_local' to avoid redownloading models you have already in the cache. You should "
...
TRANSFORMERS_DYNAMIC_MODULE_NAME = "transformers_local_modules"
...
PT_TOKEN_CLASSIFICATION_SAMPLE = r"""
    ...
    ```python
    >>> from transformers_local import {processor_class}, {model_class}
    >>> import torch
...
def hf_bucket_url(
    ...
    its sha1 if stored in git, or its sha256 if stored in git-lfs. Files cached locally from transformers_local before v3.5.0
...
def get_cached_models(cache_dir: Union[str, Path] = None) -> List[Tuple]:
    ...
    Args:
        cache_dir (`Union[str, Path]`, *optional*):
            The cache directory to search for models within. Will default to the transformers_local cache if unset.
...
def http_user_agent(user_agent: Union[Dict, str, None] = None) -> str:
    ...
    """
    ua = f"transformers_local/{__version__}; python/{sys.version.split()[0]}; session_id/{SESSION_ID}"
    if is_torch_available():
...
def get_file_from_repo(
    ...
    Args:
        ...
        use_auth_token (`str` or *bool*, *optional*):
            ...
            when running `transformers_local-cli login` (stored in `~/.huggingface`).
...
class PushToHubMixin:
        ...
        **model_card_kwargs
        ...
        Parameters:
            ...
            use_auth_token (`bool` or `str`, *optional*):
                The token to use as HTTP bearer authorization for remote files. If `True`, will use the token generated
                when running `transformers_local-cli login` (stored in `~/.huggingface`). Will default to `True` if
        ...
        ```python
        from transformers_local import {object_class}
    ...
    def _get_repo_url_from_name(
        ...
        if isinstance(use_auth_token, str):
            ...
            if token is None:
                raise ValueError(
                    "You must login to the Hugging Face hub on this computer by typing `transformers_local-cli login` and "
