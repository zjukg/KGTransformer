...
def run_hp_search_ray(trainer, n_trials: int, direction: str, **kwargs) -> BestRun:
    ...
    def _objective(trial, local_trainer, checkpoint_dir=None):
        try:
            from transformers_local.utils.notebook import NotebookProgressCallback
    ...
    def dynamic_modules_import_trainable(*args, **kwargs):
        ...
        Without this, an ImportError will be thrown. See https://github.com/huggingface/transformers_local/issues/11565.
...
class CometCallback(TrainerCallback):
    ...
    def setup(self, args, state, model):
        ...
        if state.is_world_process_zero:
            ...
            if comet_mode == "ONLINE":
                ...
                experiment.log_other("Created from", "transformers_local")
            ...
            if experiment is not None:
                experiment._set_model_graph(model, framework="transformers_local")
                experiment._log_parameters(args, prefix="args/", framework="transformers_local")
                if hasattr(model, "config"):
                    experiment._log_parameters(model.config, prefix="config/", framework="transformers_local")
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            if experiment is not None:
                experiment._log_metrics(logs, step=state.global_step, epoch=state.epoch, framework="transformers_local")
