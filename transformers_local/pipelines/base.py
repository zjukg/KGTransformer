    ...
    if isinstance(model, str):
        ...
        if config.architectures:
            ...
            for architecture in config.architectures:
                transformers_local_module = importlib.import_module("transformers_local")
                if look_pt:
                    _class = getattr(transformers_local_module, architecture, None)
                    if _class is not None:
                        classes.append(_class)
                if look_tf:
                    _class = getattr(transformers_local_module, f"TF{architecture}", None)
                    if _class is not None:
                        classes.append(_class)
...
PIPELINE_INIT_ARGS = r"""
    Arguments:
        ...
        batch_size (`int`, *optional*, defaults to 1):
            ...
            pipelines](https://huggingface.co/transformers_local/main_classes/pipelines.html#pipeline-batching) .
...
if is_torch_available():
    from transformers_local.pipelines.pt_utils import (
...
class Pipeline(_ScikitCompat):
    ...
    def transform(self, X):
        """
        Scikit / Keras interface to transformers_local' pipelines. This method will forward to __call__().
        """
        return self(X=X)
