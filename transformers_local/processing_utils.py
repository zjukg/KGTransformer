...
spec = importlib.util.spec_from_file_location(
    "transformers_local", Path(__file__).parent / "__init__.py", submodule_search_locations=[Path(__file__).parent]
)
transformers_local_module = spec.loader.load_module()
...
class ProcessorMixin(PushToHubMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for attribute_name, arg in kwargs.items():
            ...
            if isinstance(class_name, tuple):
                proper_class = tuple(getattr(transformers_local_module, n) for n in class_name if n is not None)
            else:
                proper_class = getattr(transformers_local_module, class_name)
    ...
    def _get_arguments_from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        for attribute_name in cls.attributes:
            ...
            if isinstance(class_name, tuple):
                classes = tuple(getattr(transformers_local_module, n) if n is not None else None for n in class_name)
                ...
                attribute_class = getattr(transformers_local_module, class_name)
