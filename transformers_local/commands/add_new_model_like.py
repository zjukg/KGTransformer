...
from transformers_local.models.auto.configuration_auto import model_type_to_module_name
...
def get_module_from_file(module_file: Union[str, os.PathLike]) -> str:
    ...
    # Find the first part named transformers_local, starting from the end.
    idx = len(module_parts) - 1
    while idx >= 0 and module_parts[idx] != "transformers_local":
        idx -= 1
    if idx < 0:
        raise ValueError(f"{module_file} is not a transformers_local module.")
...
class AddNewModelLikeCommand(BaseTransformersCLICommand):
    ...
    def run(self):
        if self.path_to_repo is not None:
            ...
            TRANSFORMERS_PATH = REPO_PATH / "src" / "transformers_local"
