...
``` [lfs "customtransfer.multipart"]
 path = /path/to/transformers_local/.env/bin/python args = -m debugpy --listen 5678 --wait-for-client
/path/to/transformers_local/src/transformers_local/commands/transformers_local_cli.py lfs-multipart-upload ``` """
...
class LfsCommands(BaseTransformersCLICommand):
    ...
    1. $ transformers_local-cli lfs-enable-largefiles
    ...
    2. $ transformers_local-cli lfs-multipart-upload
...
class LfsEnableCommand:
    ...
    def run(self):
        warnings.warn(
            "Managing repositories through transformers_local-cli is deprecated. Please use `huggingface-cli` instead."
        )
        ...
        if not os.path.isdir(local_path):
            ...
            exit(1)
        subprocess.run(
            "git config lfs.customtransfer.multipart.path transformers_local-cli".split(), check=True, cwd=local_path
        )
        subprocess.run(
