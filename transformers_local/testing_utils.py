...
from transformers_local import logging as transformers_local_logging
...
class CaptureLogger:
    ...
    ```python
    >>> from transformers_local import logging
    >>> from transformers_local.testing_utils import CaptureLogger

    >>> msg = "Testing 1, 2, 3"
    >>> logging.set_verbosity_info()
    >>> logger = logging.get_logger("transformers_local.models.bart.tokenization_bart")
...
def LoggingLevel(level):
    """
    This is a context manager to temporarily change transformers_local modules logging level to the desired value and have it
    ...
    ```
    """
    orig_level = transformers_local_logging.get_verbosity()
    try:
        transformers_local_logging.set_verbosity(level)
        yield
    finally:
        transformers_local_logging.set_verbosity(orig_level)
...
class TestCasePlus(unittest.TestCase):
    ...
    - `pathlib` objects (all fully resolved):
       ...
       - `src_dir` - the directory of `src` (i.e. where the `transformers_local` sub-dir resides)
...
def nested_simplify(obj, decimals=3):
    ...
    from transformers_local.tokenization_utils import BatchEncoding
