...
def keras_serializable(cls):
    ...
    1. Adding a `transformers_local_config` dict to the Keras config dictionary in `get_config` (called by Keras at
       serialization time.
    2. Wrapping `__init__` to accept that `transformers_local_config` dict (passed by Keras at deserialization time) and
...
class TFPreTrainedModel(tf.keras.Model, TFModelUtilsMixin, TFGenerationMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        ```python
        >>> from transformers_local import BertConfig, TFBertModel
